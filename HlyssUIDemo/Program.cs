...
namespace HlyssUIDemo
{
    class Program
    {
        ...
        public static void GetLonczer(HlyssApp app)
        {
            caption = "DibrySoft Launcher";

            app.Root.Children = new List<Component>()
            {
                new Panel()
                {
                    Width = "270px",
            ...
            app.Root.FindChild("nav").DefaultStyle = new Style()
            {
                {"border-radius", "0" },
                {"border-thickness", "0" },
                { "text-color", "ffffff"},
            ...
            app.Root.FindChild("header").DefaultStyle = new Style()
            {
                {"character-size", "25" }
            };

            app.Root.FindChild("user_img").DefaultStyle = new Style()
            {
                {"border-radius", "100" }
            };

            app.Root.FindChild("user_menu_btn").HoverStyle = new Style()
            {
                {"text-color", "secondary +40" }
            };

            app.Root.FindChild("user_menu_btn").PressedStyle = new Style()
            {
                {"text-color", "secondary +20" }
            };

            app.Root.FindChild("list").DefaultStyle = new Style()
            {
                {"primary-color", "00ffffff" },
                {"border-radius", "5" }
            };

            app.Root.FindChild("l1").HoverStyle =
            app.Root.FindChild("l2").HoverStyle =
            app.Root.FindChild("l3").HoverStyle =
            app.Root.FindChild("l4").HoverStyle = new Style()
            {
                {"primary-color", "11ffffff" }
            };

            app.Root.FindChild("l1").PressedStyle =
            app.Root.FindChild("l2").PressedStyle =
            app.Root.FindChild("l3").PressedStyle =
            app.Root.FindChild("l4").PressedStyle = new Style()
            {
                {"primary-color", "22ffffff" }
            };

            (app.Root.FindChild("toggle") as ToggleSwitch).Toggled += (object sender, bool toggled) =>
            {
