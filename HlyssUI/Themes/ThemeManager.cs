...
namespace HlyssUI.Themes
{
    static public class ThemeManager
    {
        ...
        public static event ThemeLoadedHandler OnThemeChanged;

        private static Dictionary<string, Theme> _themes = new Dictionary<string, Theme>();
        private static Dictionary<string, Color> _knownColors = new Dictionary<string, Color>()
        {
        ...
        public static bool IsCurrentThemeDark
        {
            get
            {
                return _themes[Name].IsDarkTheme;
            }
        }
        ...
        public static Color GetColor(string color)
        {
            ...
            if(_themes[Name].ContainsKey(color))
                return _themes[Name][color].GetModified(IsCurrentThemeDark ? modifier * -1 : modifier);
            else if (_knownColors.ContainsKey(color))
                return _knownColors[color].GetModified(IsCurrentThemeDark ? modifier * -1 : modifier);
            else
                return StringToColor(color).GetModified(IsCurrentThemeDark ? modifier * -1 : modifier);
        }

        public static void SetColor(string name, Color color)
        {
            if (_knownColors.ContainsKey(name))
                _knownColors[name] = color;
        }

        public static void AddColor(string name, Color color)
        {
            if (!_knownColors.ContainsKey(name))
                _knownColors.Add(name, color);
        }
        ...
        public static void LoadFromString(string themeStr)
        {
            XmlDocument themeXml = new XmlDocument();
            themeXml.LoadXml(themeStr);

            foreach (XmlNode singleTheme in themeXml.SelectNodes("/themes/theme"))
            {
                string name = (singleTheme.Attributes["name"] != null) ? singleTheme.Attributes["name"].Value : string.Empty;
            
                if(!_themes.ContainsKey(name) && !string.IsNullOrEmpty(name))
                {
                    Theme theme = new Theme(singleTheme);
        ...
        public static Color StringToColor(string color)
        {
            ...
            {
                Console.WriteLine($"Invalid color ({color}) [StringToColor]");
                return Color.White;
            }
        }
    }
